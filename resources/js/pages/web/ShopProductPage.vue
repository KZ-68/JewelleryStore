...
<template>
    ...
    >
        <div class="bg-gray-100 min-h-screen dark:bg-gray-950">
            <div class="max-w-screen-xl mx-auto px-4 md:px-8 lg:px-12 py-8">

                <div class="mb-6">
                    <h1 class="text-2xl md:text-3xl font-semibold text-gray-900 dark:text-gray-100">{{ props.product.name }}</h1>
                    <p v-if="props.seller_name !== null" class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        {{ useTrans('Sold by') }} <span class="font-medium">{{ props.seller_name }}</span>
                    </p>
                </div>

                <section aria-label="Product details" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 md:p-8 dark:bg-gray-900 dark:border-gray-800">
                    <div class="flex flex-col lg:flex-row gap-8 lg:gap-12">

                        <div class="w-full lg:w-[420px] shrink-0 flex flex-col gap-3">

                            <figure class="w-full aspect-square rounded-xl bg-gray-50 border border-gray-100 overflow-hidden flex items-center justify-center dark:bg-gray-800 dark:border-gray-700">
                                <img
                                    :src="mainImage"
                                    ...
                                    >
                                        ...
                                        >
                                            <figure
                                                ...
                                                :class="activeIndex === index ? 'border-[#84070F]' : 'border-gray-100 dark:border-gray-700'"
                                            >
                                                <img
                                                    :src="image"
                            ...
                            <div v-else class="grid grid-cols-3 gap-2">
                                <figure
                                    v-for="n in 3"
                                    :key="n"
                                    class="w-full aspect-square rounded-lg bg-gray-50 border border-gray-100 overflow-hidden flex items-center justify-center dark:bg-gray-800 dark:border-gray-700"
                                >
                        ...
                        <div class="flex-1 flex flex-col gap-6">
                            ...
                            <div class="flex items-baseline gap-2">
                                <span class="text-3xl font-bold text-red-900 dark:text-red-400">{{ props.price }}</span>
                                <span class="text-red-800 text-lg dark:text-red-400">€</span>
                                <span class="ml-2 text-sm text-gray-400 dark:text-gray-500">{{ useTrans('All taxes included') }}</span>
                            </div>
                            ...
                            <div class="flex flex-wrap items-center gap-4">
                                <div class="relative">
                                    <label for="product-quantity" class="absolute top-1.5 left-4 text-xs uppercase text-gray-400 tracking-wide font-semibold dark:text-gray-500">
                                        {{ useTrans('Qty') }}
                                    </label>
                                    <select
                                        id="product-quantity"
                                        ...
                                        aria-label="quantity"
                                        class="cursor-pointer appearance-none rounded-xl border border-gray-200 pl-4 pr-8 pt-6 pb-2 focus:outline-none focus:ring-2 focus:ring-[#84070F] dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200"
                                    >
                            ...
                            <Link
                                ...
                                class="inline-flex items-center justify-center rounded-xl border border-gray-800 px-5 py-3 text-sm font-medium text-gray-800 hover:bg-gray-800 hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-gray-800 w-fit dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white"
                            >
                                {{ useTrans('Contact the seller') }}
                            </Link>

                            <div class="border-t border-gray-100 pt-5 mt-2 dark:border-gray-800">
                                <ul class="grid grid-cols-1 sm:grid-cols-2 gap-3" role="list" aria-label="Garanties">
                                    <li class="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-400">
                                        ...
                                        <div>
                                            <p class="font-semibold text-gray-800 dark:text-gray-200">{{ useTrans('Secured Delivery') }}</p>
                                            <p class="text-xs text-gray-400 dark:text-gray-500">{{ useTrans('Tracked and protected package')}}</p>
                                        </div>
                                    </li>
                                    <li class="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-400">
                                        <svg aria-hidden="true" class="w-5 h-5 text-[#84070F] shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                        ...
                                        <div>
                                            <p class="font-semibold text-gray-800 dark:text-gray-200">{{ useTrans('Secured Payment')}}</p>
                                            <p class="text-xs text-gray-400 dark:text-gray-500">{{useTrans('100% secure transactions')}}</p>
                                        </div>
                                    </li>
                                    <li class="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-400">
                                        <svg aria-hidden="true" class="w-5 h-5 text-[#84070F] shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                        ...
                                        <div>
                                            <p class="font-semibold text-gray-800 dark:text-gray-200">{{useTrans('Returns within 30 days')}}</p>
                                            <p class="text-xs text-gray-400 dark:text-gray-500">{{useTrans('Money-back guarantee')}}</p>
                                        </div>
                                    </li>
                                    <li class="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-400">
                                        <svg aria-hidden="true" class="w-5 h-5 text-[#84070F] shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                        ...
                                        <div>
                                            <p class="font-semibold text-gray-800 dark:text-gray-200">{{useTrans('Customer service')}}</p>
                                            <p class="text-xs text-gray-400 dark:text-gray-500">{{useTrans('Available 7 days a week')}}</p>
                                        </div>
                                    </li>
                                </ul>
                            </div>
                ...
                <section aria-label="Informations produit" class="mt-8">
                    ...
                    <div role="tablist" aria-label="Product section" class="flex gap-1 border-b border-gray-200 dark:border-gray-700">
                        <button
                            id="tab-description"
                            role="tab"
                    ...
                    <div
                        ...
                        class="bg-white rounded-b-xl rounded-tr-xl border border-t-0 border-gray-100 min-h-48 p-6 lg:p-8 dark:bg-gray-900 dark:border-gray-800"
                    >
                    ...
                    <div
                        ...
                        class="bg-white rounded-b-xl rounded-tr-xl border border-t-0 border-gray-100 min-h-48 p-6 lg:p-8 dark:bg-gray-900 dark:border-gray-800"
                    >
                        <p class="text-gray-500 text-sm dark:text-gray-400">{{ useTrans('No features listed.') }}</p>
                    </div>
